...
{
    public class PayrollBaseRepository<T> : IBaseRepository<T> where T : class
    {
        private readonly IPayrollUnitOfWork _payrollDbContext;
        internal DbSet<T> dbSet = null;
        public PayrollBaseRepository(IPayrollUnitOfWork payrollUnitOfWork)
        {
            if (payrollUnitOfWork == null) throw new ArgumentNullException("Payroll DbContext is not assigned");
            _payrollDbContext = payrollUnitOfWork;
            dbSet = _payrollDbContext.Db.Set<T>();
        }
        ...
        public bool Save()
        {
            return _payrollDbContext.Db.SaveChanges() > 0;
        }
        public async Task<bool> SaveAsync()
        {
            return await _payrollDbContext.Db.SaveChangesAsync() > 0;
        }
        ...
        {
            using (var cmd = _payrollDbContext.Db.Database.GetDbConnection().CreateCommand())
            {
                cmd.CommandText = Sql;
        ...
        {
            _payrollDbContext.Db.Entry(entity).State = EntityState.Modified;
        }
        ...
        public Task UpdateAllAsync(IList<T> entities)
        {
        ...
        {
            throw new NotImplementedException();
            //await _payrollDbContext.Db.Entry(entity).State = EntityState.Modified;
        }
