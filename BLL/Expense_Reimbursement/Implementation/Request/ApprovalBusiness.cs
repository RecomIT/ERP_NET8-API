...
namespace BLL.Expense_Reimbursement.Implementation.Request
{
    public class ApprovalBusiness : IApprovalBusiness
    {
        ...
        public async Task<IEnumerable<RequestCountViewModel>> GetRequestCountAdvanceAsync(long authorityId, AppUser user)
        {
            IEnumerable<RequestCountViewModel> data = new List<RequestCountViewModel>();
            try
            {
                var sp_name = "sp_Reimburse_Request_Count_Account";
                var parameters = new DynamicParameters();
                parameters.Add("UserId", authorityId);
                parameters.Add("CompanyId", user.CompanyId);
                parameters.Add("OrganizationId", user.OrganizationId);
                parameters.Add("ExecutionFlag", "Approved");

                data = await _dapper.SqlQueryListAsync<RequestCountViewModel>(user.Database, sp_name, parameters, CommandType.StoredProcedure);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IEnumerable<RequestCountViewModel>> GetRequestCountAsync(long authorityId, AppUser user)
        {
            IEnumerable<RequestCountViewModel> data = new List<RequestCountViewModel>();
            //var sp_name = "";
            try
            {
                //if (user.RoleName == "Account")
                //{
                //    sp_name = "sp_Reimburse_Request_Count_Account";
                //}
                //else
                //{
                //    sp_name = "sp_Reimburse_Request_Count";
                //}

                var sp_name = "sp_Reimburse_Request_Count";
                var parameters = new DynamicParameters();
                parameters.Add("UserId", authorityId);
                parameters.Add("CompanyId", user.CompanyId);
                parameters.Add("OrganizationId", user.OrganizationId);
        ...
        public async Task<DBResponse<RequestViewModel>> GetRequestDataAdvanceAsync(RequestFilter filter, AppUser user)
        {
            DBResponse<RequestViewModel> data = new DBResponse<RequestViewModel>();
            DBResponse response = new DBResponse();
            try
            {
                var sp_name = "sp_Reimburse_Request_Approval_List_Account";
                var parameters = DapperParam.AddParams(filter, user, addBaseProperty: true);
                //filter.StatusChange == 1 ||
                if (filter.Status == "Payment" || filter.StateStatus == "Payment")
                {
        ...
        public async Task<DBResponse<RequestViewModel>> GetRequestDataAsync(RequestFilter filter, AppUser user)
        {
            ...
            try
            {
                var sp_name = "sp_Reimburse_Request_Approval_List";
                var parameters = DapperParam.AddParams(filter, user, addBaseProperty: true);
                //filter.StatusChange == 1 ||
                if (filter.Status == "Approved" || filter.Status == "Rejected" || filter.StateStatus == "Approved" || filter.StateStatus == "Rejected")
                {
                    parameters.Add("ExecutionFlag", "Show");     
                }
                else
        ...
        public async Task<IEnumerable<RequestViewModel>> GetRequestDetailsDataAsync(RequestFilter filter, AppUser user)
        {
        ...
        public async Task<ExecutionStatus> ApprovedRequestAsync(ApprovedDTO model, AppUser user)
        {
            ExecutionStatus executionStatus = new ExecutionStatus();
            try
            {
                var sp_name = "sp_Reimburse_Request_Approved";
                var parameters = new DynamicParameters();
                ...
                parameters.Add("SpendMode", model.SpendMode);
                parameters.Add("CommentsUser", model.CommentsUser);
                parameters.Add("CommentsAccount", model.CommentsAccount);

                parameters.Add("UserId", user.UserId);
                parameters.Add("CompanyId", user.CompanyId);
                parameters.Add("OrganizationId", user.OrganizationId);
                
                if (model.Flag == "Approved") 
                {
                    parameters.Add("ExecutionFlag", "Approved");
                }
                if (model.Flag == "Rejected")
                {
                    parameters.Add("ExecutionFlag", "Rejected");
                }

                if (user.RoleName == "Account")
                {
                    parameters.Add("ExecutionFlag", "Payment");
                }
            ...
            {
                await _sysLogger.SaveHRMSException(ex, user.Database, "ApprovalBusiness", "ApprovedRequestAsync", user);
            }
        ...
        public async Task<IEnumerable<EmailSendViewModel>> EmailSendAsync(EmailDataViewModel request, AppUser user)
        {
            ...
            try
            {
                var sp_name = "sp_Reimburse_Request_Approved";
                var parameters = new DynamicParameters();
                parameters.Add("@SpendMode", request.SpendMode);
        ...
        private async Task EmailSend(EmailSendViewModel email, EmailSettingViewModel emailSetting, EmailSendViewModel emailDtl, EmailDataViewModel request)
        {
            ...
            try
            {
                ...
                if (request.Flag == "Approved")
                {
                    ...
                    {
                        Subject = "Approved " + request.TransactionType + " Expenses Request";
                    }
                   
                    message.To.Add(new MailAddress(emailDtl.EmailTo));
                }
                else if (request.Flag == "Rejected")
                {
                    if (request.SpendMode == "Advance")
                    {
                        Subject = "Rejected Advance Payment Request For " + request.TransactionType + ".";
                    }
                    else
                    {
                        Subject = "Rejected " + request.TransactionType + " Expenses Request";
                    }

                    message.To.Add(new MailAddress(emailDtl.UserEmail));
                }
                ...
                {
                    Subject = "Payment " + request.TransactionType + " Expenses Request";
                    message.To.Add(new MailAddress(emailDtl.EmailTo));
                }
                if (request.Flag == "Settlement")
                {
                    Subject = "Settlement " + request.TransactionType + " Expenses Request";
                    message.To.Add(new MailAddress(emailDtl.EmailTo));
                }
        ...
        //        if (model.EntertainmentId > 0 && model.File != null)
        //        {
        ...
        //        }
        //        else if (model.EntertainmentId == 0 && model.File != null)
        //        {
        //            file = await Upload.SaveFileAsync(model.File, string.Format(@"{0}", user.OrgCode), model.TransactionType, string.Format(@"{0}", model.EmployeeId), model.ReferenceNumber);
        //            filePath = file.Substring(0, file.LastIndexOf("/"));
        //            fileName = file.Substring(file.LastIndexOf("/") + 1);
        ...
        //        var sp_name = "";
        //        var parameters = new DynamicParameters();
        //        sp_name = $@"Update Reimburse_Entertainment Set [FileName]=@fileName,ActualFileName=@actualFileName,FileSize=@fileSize,
        //            FileFormat=@extenstion,FilePath=@filePath
        //            WHERE EmployeeId=@EmployeeId AND CompanyId=@CompanyId AND OrganizationId=@OrganizationId";               

        //        parameters.Add("EntertainmentId", model.EntertainmentId);
        //        parameters.Add("EmployeeId", model.EmployeeId);
