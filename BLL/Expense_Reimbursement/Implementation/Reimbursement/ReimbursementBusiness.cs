...
namespace BLL.Expense_Reimbursement.Implementation.Reimbursement
{
    public class ReimbursementBusiness : IReimbursementBusiness
    {
        private readonly IDapperData _dapper;
        private readonly ISysLogger _sysLogger;

        public ReimbursementBusiness(
            IDapperData dapper,
        ...
        public async Task<IEnumerable<RequestCountViewModel>> GetRequestCountReimbursementAsync(long authorityId, AppUser user)
        {
            ...
            try
            {
                ...
                parameters.Add("ExecutionFlag", "Reimbursement");
        ...
        public async Task<DBResponse<RequestViewModel>> GetRequestDataReimbursementAsync(RequestFilter filter, AppUser user)
        {
            ...
            try
            {
                ...
                if (filter.Status == "Payment")
                {
                    parameters.Add("ExecutionFlag", "ReimbursementShow");
                }
                else
                {
                    parameters.Add("ExecutionFlag", "Reimbursement");
        ...
        private async Task EmailSend(EmailSendViewModel email, EmailSettingViewModel emailSetting, EmailSendViewModel emailDtl, EmailDataViewModel request)
        {
            ...
            try
            {
                ...
                {
                    Subject = "Expenses Approval Request";
                    message.To.Add(new MailAddress(emailDtl.EmailTo));
                }
                else if (request.Flag == "Edit")
                {
                    Subject = "Requested Expenses has been Edited";
                    message.To.Add(new MailAddress(emailDtl.EmailTo));
                }
                else if (request.Flag == "Cancel")
                {
                    Subject = "Requested Expenses has been Cancelled";
                    message.To.Add(new MailAddress(emailDtl.EmailTo));
                }
